...
RenderEngine* RenderEngine::create(EGLContext ctx) {
    ...
    const EGLint eglAttribs[] = { EGL_BUFFER_SIZE, 32, EGL_ALPHA_SIZE, 8, EGL_BLUE_SIZE, 8, EGL_GREEN_SIZE, 8, EGL_RED_SIZE, 8, EGL_RENDERABLE_TYPE, EGL_OPENGL_ES2_BIT,
...
#warning "using EGL_IMG_context_priority"
    ...
    if (dummyConfig == EGL_NO_CONFIG) {
        dummyConfig = chooseEglConfig(display, 1);
    }
    ...
    engine->setDefaultDisplay(display);
...
class EGLAttributeVector {
    ...
    class Adder;
    friend class Adder;
    typedef std::map<Attribute, EGLint> AtrribMap;
    AtrribMap mList;
    struct Attribute {
        Attribute() {};
    ...
    public:
        void operator = (EGLint value) {
            if (attribute != EGL_NONE) {
                v.mList.insert(AtrribMap::value_type(attribute, value));
            }
        }
...
public:
    EGLAttributeVector() {
        mList.insert(AtrribMap::value_type(EGL_NONE, EGL_NONE));
    }
    void remove(EGLint attribute) {
        if (attribute != EGL_NONE) {
            mList.erase(attribute);
        }
    }
    ...
    EGLint operator [] (EGLint attribute) const {
       return mList.find(attribute)->second;
    }
    // cast-operator to (EGLint const*)
    operator EGLint const* () const { return & (mList.begin()->second); }
};
...
EGLConfig RenderEngine::chooseEglConfig(EGLDisplay display, int format) {
    ...
    void RenderEngine::release() {
        eglMakeCurrent(mDefaultdpy, EGL_NO_SURFACE, EGL_NO_SURFACE, EGL_NO_CONTEXT);
        delete this;
    }

    EGLSurface RenderEngine::createSurface(int w, int h) {
        ...
        return eglCreatePbufferSurface(mDefaultdpy, mEGLConfig, pBufferAttrib);
    }

    void RenderEngine::makeCurrent(EGLSurface surface) {
        eglMakeCurrent(eglGetDisplay(EGL_DEFAULT_DISPLAY), surface, surface, mEGLContext);
    }

    void RenderEngine::setDefaultDisplay(EGLDisplay pVoid) {
        mDefaultdpy = pVoid;
    }

    void RenderEngine::destroySurface(EGLSurface surface) {
